...
function evaluate(ctx = {}) {
  ...
  // CALIBRATED 2026-05-19: per user spec — scalp family gets PRIORITY over
  // non-scalp playbooks of the same conviction tier. The user wants the
  // engine to "go through all strategies, finalize the entry, with high
  // priority on scalpings". This boost ensures that when both a scalp
  // and a swing setup qualify, the scalp wins. The institutional WR is
  // preserved because both must reach elite/standard conviction to be
  // valid in the first place.
  //
  // Scalp identification: holdProfile.tradeType === 'SCALP', OR family
  // contains 'scalp' / 'reclaim' / 'oscillation' / 'rotation'.
  const _isScalp = (e) => {
    if (e?.holdProfile?.tradeType === 'SCALP') return true;
    const f = (e?.family || '').toLowerCase();
    if (f === 'momentum_continuation') return false;     // SWING by default
    if (f === 'breakout_expansion') return false;        // SWING by default
    if (e?.name && /SCALP|RECLAIM|OSCILL|ROTATION|FADE|REVERSION|FLIP/.test(e.name)) return true;
    return false;
  };

  final.sort((a, b) => {
    // 1) Conviction tier always wins
    const rank = { elite: 3, standard: 2, weak: 1 };
    const ra = rank[a.conviction] || 0;
    const rb = rank[b.conviction] || 0;
    if (rb !== ra) return rb - ra;
    // 2) Within the same tier, scalps win over non-scalps
    const sa = _isScalp(a) ? 1 : 0;
    const sb = _isScalp(b) ? 1 : 0;
